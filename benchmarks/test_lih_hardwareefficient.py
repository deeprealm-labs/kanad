"""Test LiH with HardwareEfficient ansatz"""
import time
import numpy as np
...
mol = Molecule(atoms=[Atom('Li', [0,0,0]), Atom('H', [0,0,1.595])], charge=0)
...
pauli_h = openfermion_jordan_wigner(h_mo, eri_mo, mol.hamiltonian.nuclear_repulsion, 4 // 2)
E_fci = np.linalg.eigh(pauli_h.to_matrix())[0][0]

# VQE
ansatz = HardwareEfficientAnsatz(n_qubits=12, n_electrons=4, n_layers=2, entanglement='linear')
...
print(f"LiH/HardwareEfficient/SLSQP: E={E_vqe:.6f} Ha, error={err:+.3f} mHa, time={dt:.1f}s")
