"""Test H2 with HardwareEfficient ansatz - SLSQP optimizer"""
import time
import numpy as np
...
# H2 molecule
mol = Molecule(atoms=[Atom('H', [0,0,0]), Atom('H', [0,0,0.735])], charge=0)
...
pauli_h = openfermion_jordan_wigner(h_mo, eri_mo, mol.hamiltonian.nuclear_repulsion, 1)
E_fci = np.linalg.eigh(pauli_h.to_matrix())[0][0]

# VQE
ansatz = HardwareEfficientAnsatz(n_qubits=4, n_electrons=2, n_layers=2, entanglement='linear')
...
print(f"H2/HardwareEfficient/SLSQP: E={E_vqe:.6f} Ha, error={err:+.3f} mHa, time={dt:.1f}s")
assert abs(err) < 10, f"Error too large: {err} mHa"
print("✓ PASS")
